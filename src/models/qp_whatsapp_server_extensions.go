...
func PostToDispatchingFromServer(server *QpWhatsappServer, message *whatsapp.WhatsappMessage, from string) (err error) {
	if server == nil {
	...
	for _, dispatching := range server.QpDataDispatching.Dispatching {
		...
		if !message.FromInternal || (dispatching.ForwardInternal && (len(dispatching.TrackId) == 0 || dispatching.TrackId != message.TrackId)) {
			elerr := dispatching.Dispatch(message, from)
			if elerr != nil {
